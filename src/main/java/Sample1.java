...
public class Sample1 {
	...
	public static void main1 (String[] args) throws Exception {
		...
		/*
		------------使用示例�?：验证回调URL---------------
		*企业�?启回调模式时，企业号会向验证url发�?�一个get请求 
		假设点击验证时，企业收到类似请求�?
		* GET /cgi-bin/wxpush?msg_signature=5c45ff5e21c57e6ad56bac8758b79b1d9ac89fd3&timestamp=1409659589&nonce=263014780&echostr=P9nAzCzyDtyTWESHep1vC5X9xho%2FqYX3Zpb4yKa9SKld1DsH3Iyt3tP3zNdtp%2B4RPcs8TgAE7OaBO%2BFZXvnaqQ%3D%3D 
		* HTTP/1.1 Host: qy.weixin.qq.com

		接收到该请求时，企业�?		1.解析出Get请求的参数，包括消息体签�?(msg_signature)，时间戳(timestamp)，随机数字串(nonce)以及公众平台推�?�过来的随机加密字符�?(echostr),
		这一步注意作URL解码�?
		2.验证消息体签名的正确�? 
		...
		*/
		...
		企业收到post请求之后应该		1.解析出url上的参数，包括消息体签名(msg_signature)，时间戳(timestamp)以及随机数字�?(nonce)
		2.验证消息体签名的正确性�??
		3.将post请求的数据进行xml解析，并�?<Encrypt>标签的内容进行解密，解密出来的明文即是用户回复消息的明文，明文格式请参�?�官方文�?
		�?2�?3步可以用公众平台提供的库函数DecryptMsg来实现�??
		*/
	...
	/*
		------------使用示例三：企业回复用户消息的加�?---------------
		企业被动回复用户的消息也�?要进行加密，并且拼接成密文格式的xml串�??
		假设企业�?要回复用户的明文如下�?
		<xml>
		...
		为了将此段明文回复给用户，企业应�?			1.自己生成时间时间�?(timestamp),随机数字�?(nonce)以便生成消息体签名，也可以直接用从公众平台的post url上解析出的对应�?��??
